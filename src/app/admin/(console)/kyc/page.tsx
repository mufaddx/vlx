        ...
        {rows.map((r) => (
          <li key={r.id} className="flex items-center justify-between rounded-2xl border border-mist-200 p-3 dark:border-white/10">
            ...
            <div className="flex gap-2">
              <form action={asFormAction(adminKycAction.bind(null, r.userId, "approved"))}><button className="btn-primary" type="submit">Approve</button></form>
              <form action={asFormAction(adminKycAction.bind(null, r.userId, "rejected"))}><button className="btn-secondary" type="submit">Reject</button></form>
            </div>
          </li>
        ))}
      </ul>
